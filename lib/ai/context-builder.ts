...
export interface BuiltAgentContext {
  ...
  retrieval: { projectId: string; episodeId?: string; activeSceneId?: string; includedSceneIds: string[]; neighboringSceneIds: string[]; fullScriptSceneIds: string[]; complete: false };
...
export function buildAgentContext(options: BuildContextOptions): BuiltAgentContext {
  const {
    rootProject,
    activeWorkspace,
  ...
  if (isEpisode) {
    ...
    contextTagParts.push(`S${String(sNum).padStart(2, '0')} E${String(epNum).padStart(2, '0')}`);
  } else {
...
Episode Synopsis: ${currentProj.synopsis || 'None'}`);
  ...
  if (activeView === 'Screenplay' || activeView === 'Story') {
    if (activeScene) {
      const prevScene = currentSceneIndex > 0 ? allScenes[currentSceneIndex - 1] : undefined;
      const nextScene = currentSceneIndex < allScenes.length - 1 ? allScenes[currentSceneIndex + 1] : undefined;

      includedSceneIds.push(activeScene.id);
      fullScriptSceneIds.push(activeScene.id);
      for (const neighbor of [prevScene, nextScene]) if (neighbor) { includedSceneIds.push(neighbor.id); neighboringSceneIds.push(neighbor.id); }
      const sceneChars = (currentProj.characters || [])
        .filter((c) => (activeScene?.characterIds || []).includes(c.id))
        .map((c) => sanitizeCharacter(c));

      const sceneLocation = (currentProj.locations || []).find((l) => l.id === activeScene?.locationId);
...
${activeScene.blocks.map((b) => `[${b.type.toUpperCase()}] ${b.content}`).join('\n')}`;
      ...
      if (prevScene) {
        sceneText = `=== PREVIOUS SCENE (${prevScene.heading}) ===\nSummary: ${prevScene.summary || 'None'}\n\n` + sceneText;
      }
      if (nextScene) {
...
Scene List:
${allScenes.slice(0, 5).map((s) => `- ${s.heading} (${s.summary || 'No summary'})`).join('\n')}`);
    }
  } else if (activeView === 'Characters' || activeView === 'Cast & Crew') {
    const chars = (currentProj.characters || []).map((c) => sanitizeCharacter(c));
...
Scenes: ${allScenes.length} | Characters: ${(currentProj.characters || []).length} | Locations: ${(currentProj.locations || []).length}`);
  ...
  // 4. Selection Context (if user highlighted text or selected blocks)
  if (selectedText || selectedBlockIds.length > 0) {
    let selectionContent = selectedText;
    if (!selectionContent && activeScene) {
      const selectedBlocks = activeScene.blocks.filter((b) => selectedBlockIds.includes(b.id));
      selectionContent = selectedBlocks.map((b) => `[${b.type.toUpperCase()}] ${b.content}`).join('\n');
    }

    if (selectionContent) {
      sections.push(`=== WRITER'S CURRENT SELECTION ===
${selectionContent}`);
    }
  }

  traceScenePayload('contextSceneSummaries.length', includedSceneIds, { contextSceneSummariesLength: includedSceneIds.length, scope: 'local', activeSceneId: activeScene?.id });
  ...
  return {
    retrieval: { projectId: rootProject.id, episodeId: isEpisode ? currentProj.id : undefined, activeSceneId: activeScene?.id, includedSceneIds, neighboringSceneIds, fullScriptSceneIds, complete: false },
