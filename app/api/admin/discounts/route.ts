...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'create') {
      ...
      const created = await store.createDiscount(discount, adminUserId, adminEmail);
      return NextResponse.json({ ok: true, discount: created });
    }

    if (action === 'update') {
      if (!discountId) {
        return NextResponse.json({ error: 'Missing discountId' }, { status: 400 });
      }
      const planConfigs = await store.getPlanConfigs();
      const validation = validateDiscount(discount, planConfigs);
      if (!validation.valid) {
      ...
      const updated = await store.updateDiscount(discountId, discount, adminUserId, adminEmail);
      return NextResponse.json({ ok: true, discount: updated });
    }

    if (action === 'toggle') {
      if (!discountId || typeof enabled !== 'boolean') {
        return NextResponse.json({ error: 'Missing discountId or enabled' }, { status: 400 });
      }
      const updated = await store.toggleDiscount(discountId, enabled, adminUserId, adminEmail);
      return NextResponse.json({ ok: true, discount: updated });
    }
